...
const RouteDrawingLayer = ({ destinations, onSaveSuccess }) => {
    const [isDrawing, setIsDrawing] = useState(false);
    const [points, setPoints] = useState([]);
    const [fromId, setFromId] = useState(null);

    // לוג לבדיקה - יופיע ב-Console בכל פעם שהמצב משתנה
    useEffect(() => {
    ...
    const handleNodeClick = (e, dest) => {
        ...
        if (!isDrawing) {
            ...
            // סיום מסלול
            if (dest.id === fromId) {
                console.log("Canceled: clicked the same animal");
                reset();
                return;
            }
