...
const DestinationEditor = ({ destinations = [], action, onSaveSuccess }) => {
    ...
    const [formData, setFormData] = useState({
        id: null, name: '', category: 'CAGES', description: '', picUrl: '', x: 0, y: 0, imageFile: null
    });

    const handleClose = () => {
        setFormData({ id: null, name: '', category: 'CAGES', description: '', picUrl: '', x: 0, y: 0, imageFile: null });
        setShowForm(false);
        setIsUpdating(false);
    };

    useEffect(() => { handleClose(); }, [action]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            payload.append('destination', new Blob([JSON.stringify(dto)], { type: 'application/json' }));
    ...
    return (
        <g className="destination-editor-layer">
            {action === 'create' && (
                <rect width="100" height="100" fill="transparent" onMouseDown={handleMapClick} style={{ cursor: 'crosshair' }} />
            )}
            ...
            {action === 'update' && destinations.map(dest => {
                ...
                return (
                    <g key={dest.id} onClick={() => loadDestinationForEditing(dest)} style={{ cursor: 'pointer' }}>
                        <circle cx={nodeX} cy={nodeY} r="5" fill="transparent" />
                        {isSelected && <circle cx={nodeX} cy={nodeY} r="3.5" fill="none" stroke="#0ea5e9" strokeWidth="0.6" />}
                        <circle cx={nodeX} cy={nodeY} r="2.2" fill={isSelected ? "#0ea5e9" : "#eab308"} stroke="#fff" strokeWidth="0.4" />
                    </g>
                );
            })}
            ...
            {ReactDOM.createPortal(
                <div style={{ position: 'relative', top: '-10px', padding: '0 10px' }}>
                    {action === 'update' && !showForm && (
                        <div style={{ padding: '15px', background: '#1e293b', borderRadius: '12px', border: '1px solid #334155', marginBottom: '50px' }}>
                            <h3 style={{ margin: '0 0 10px 0', fontSize: '16px', color: '#fff' }}>✏️ Edit Destination</h3>
                            <select 
                                onChange={(e) => loadDestinationForEditing(destinations.find(d => d.id === parseInt(e.target.value)))}
                                style={{ width: '100%', padding: '8px', background: '#0f172a', color: '#fff', border: '1px solid #475569', borderRadius: '6px' }}
                            >
                                <option value="">-- Select from list --</option>
                                {destinations.map(d => <option key={d.id} value={d.id}>{d.name || `ID: ${d.id}`}</option>)}
                            </select>
                        </div>
                    )}
